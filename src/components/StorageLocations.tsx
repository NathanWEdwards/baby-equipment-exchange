...
const StorageLocations = (props: StorageLocationsProps) => {
    ...
    useEffect(() => {
        setFilteredLocations(
            storageLocations.filter((location) => Object.values(location).some((value) => String(value).toLowerCase().includes(searchInput.toLowerCase())))
        );
    }, [searchInput]);
            ...
            {!idToDisplay && !showForm && (
                <>
                    ...
                    {storageLocations.length === 0 ? (
                        <div style={{ textAlign: 'center', marginTop: '3rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                            <Typography variant="h6" color="text.secondary" sx={{ mb: 2 }}>
                                No storage locations are available.
                            </Typography>
                            <Button variant="contained" color="primary" type="button" onClick={handleShowForm} size="large">
                                Create New Location
                            </Button>
                        </div>
                    ) : (
                                            ...
                                            <ListItem key={location.id}>
                                                ...
                                                >
                                                    ...
                                                    <ListItemText
                                                        ...
                                                        secondary={
                                                            location.address ? `${location.address.city ?? ''}, ${location.address.state ?? ''}` : 'No address'
                                                        }
