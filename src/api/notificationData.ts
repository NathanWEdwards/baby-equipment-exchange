...
export async function computeNotificationItems(data: NotificationData): Promise<NotificationItem[]> {
    ...
    // Donations needing storage assignment
    const unassignedDonations = data.donations.filter(
        (d) => !d.storage && d.status !== 'in processing' && d.status !== 'rejected' && d.status !== 'unavailable' && d.status !== 'not-received'
    );
    for (const donation of unassignedDonations) {
        items.push({
            id: `storage-${donation.id}`,
            type: 'pending-storage',
            priority: 25,
            title: `${donation.brand} - ${donation.model}`,
            subtitle: `No storage location assigned`,
            timestamp: donation.modifiedAt ? donation.modifiedAt.toDate() : new Date(),
            entityId: donation.id,
            entityType: 'donation',
            tab: 'Storage Assignment',
            tabIndex: 4,
            isNew: false
        });
    }
